...
(2 points) In the field of linear algebra, the dot product of two vectors with $n$ elements each, $\vec{a} = [a_1, a_2, ..., a_n]$ and $\vec{b} = [b_1, b_2, ..., b_n]$, can be calculated as follows:

$$
\vec{a} \cdot \vec{b} = \sum_n a_n b_n
$$

1. Write the pseudocode for a function that takes two arrays of numbers as inputs and returns their dot product.
2. What is the time complexity of your dot product algorithm?
3. What is the space complexity of your dot product algorithm?

> [!tip]- Hint!
>
> Be sure to check if the arrays have the same length before doing any calculations!

---

(2 points) Consider an $n \times n$ matrix that is represented as a two-dimensional array of integers. In the field of linear algebra, the transpose of a matrix flips a matrix over its diagonal. In other words, it switches the row and column indices of the matrix:
