...
1. Show that $f(n) = 2^{n + 1} + n$ is $O(2^n)$. Justify your answer with a proof.

---

2. Show that $f(n) = n^2 + 4n + 5$ is $\Omega (n^2)$. Justify your answer with a proof.

---

3. Show that $f(n) = 10 n log n$ is $\Theta (n log n)$. Justify your answer with a proof.

---

4. Prove by induction that $n^3 + 2n$ is divisible by $3$ for every non-negative integer n.

---

5. According to [this reference](https://www.geeksforgeeks.org/time-complexities-of-all-sorting-algorithms/), the Bubble Sort algorithm has a worst case time complexity of $O(n^2)$, but a best case time complexity of $O(n)$. Does this mean that Bubble Sort is more performant than an algorithm like Quicksort with a worst case and best case complexity of $O(n log n)$? Why don't we just use Bubble Sort in large systems, all the time?

---

6. Suppose I create a data structure called `MyCoolStructure` which has four operations. Each of the operations runs in O(1) time:
